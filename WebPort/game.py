...
class Game:
    def __init__(self):
        ...
        self.font  = pygame.font.Font("TradeWinds-Regular.ttf", S(45))
        self.assets    = Assets()
    ...
    def update(self, dt):
        ...
        if self.state == "game" and not self.menus.in_game_menu_open:
            ...
            if self.ending_cutscene_pending and not self.catch_mode.active:
                ...
                self.special_splash_rect     = pygame.Rect(WIDTH // 2 - S(60), HEIGHT // 2 - S(60), S(120), S(120))
    ...
    def _draw_gold(self):
        gold_label  = self.font.render("Gold: ", True, WHITE)
        ...
        btn_x = self.screen.get_width() - btn_w - S(20)
        btn_y = S(20)
    ...
    def _draw_game_scene(self, dt_ms):
        ...
        self.screen.blit(lake_surf, (0, 0))
        ...
        if cutscene_active:
            ...
            if self.catch_mode.active:
                ...
                if self.special_splash_active and self.special_splash_rect:
                    ...
                    self.screen.blit(splash_surf, self.special_splash_rect)
                self.fishing.draw(self.screen)
                self.rope.draw(self.screen, self.fishing.get_depth())

            self.inventory.draw_bucket(self.screen, SP(1530, 870))
            self._draw_gold()
            ...
            if current_time - self.inventory_full_timer < 2000 and (current_time // 300) % 2 == 0:
                ...
                self.screen.blit(full_surf, full_surf.get_rect(center=(WIDTH // 2, S(150))))

            if current_time - self.cheat_timer < 2000:
                cheat_surf = self.font.render("CHEAT ACTIVATED!", True, (0, 255, 100))
                self.screen.blit(cheat_surf, cheat_surf.get_rect(center=(WIDTH // 2, S(200))))
            ...
            if self.show_fps:
                ...
                self.screen.blit(fps_surf, (S(10), S(10)))
